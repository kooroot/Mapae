...
| Standing gates | Documentation, logging, advisory and test-count gates (`check:counts` re-measures the badge and the breakdown below on every run); 23/23 negative paths on both chain targets; 16/16 onboarding cases on a GIWA fork | Local + read-only GIWA verification |
...
- **A settlement can outlive the agent's patience, and then the answer is "unknown".**
  ...
  can pay twice. A seller that would not take the payment at all (503/429 — the
  facilitator was rate-limited or not ready) is `SELLER_UNAVAILABLE`: nothing charged,
  safe to retry. A fork mines instantly, so no amount of local testing would have shown
  this.
