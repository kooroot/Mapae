...
| 상시 게이트 | 문서·로깅·의존성 권고·테스트 수 상시 게이트(`check:counts`가 매번 배지와 아래 내역을 다시 셈), 두 체인 타깃 negative path 23/23, GIWA fork 온보딩 16/16 | 로컬 + GIWA 읽기 전용 검증 |
...
- **온보딩 스폰서는 인증 없는 인터넷 요청에 응답하는, 자금이 든 키다.** 키페어는
  ...
  뿐이다. 그리핑을 실제로 막는 것은 계정당 24시간 1회의 faucet 창, 일일 가스 예산,
  그리고 일부러 작게 유지하는 스폰서 지갑 잔액이다. 이걸 소진시키면 그날의 온보딩이 멈출 뿐 —
...
`FACILITATOR_SIGNER_ADDRESS must be set (apps/delegation-lab/.env)`에서 멈춥니다. 준비 절차는
...
| `FACILITATOR_SIGNER_ADDRESS` | 정산 서명자 기대 주소, Framework 배포에는 사용하지 않음 |
...
apps/delegated-seller/     ERC-7710 호스티드 상점: 매니페스트·유료 티켓·주문
