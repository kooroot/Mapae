 ...
 * stays because it is the one that answers with this service's closed enum and log line.
 * It is the smaller number so that a body the runtime lets through and the route still
 * refuses gets that answer rather than a bare 413; UTF-8 never has fewer bytes than
 * characters, so nothing between the two slips past both. A legitimate body is one
 * permission context, ~2 kB.
 */
